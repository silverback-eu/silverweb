...
>((props, ref) => {
  const {
    className, readOnly, disabled, variant, onClick, cardLabel, cardDescription, ...rest
  } = props;
  ...
  if (variant === "big") {
    return (
      <Card
        className={cn("w-full", (!readOnly && !disabled) ? " cursor-pointer" : "cursor-not-allowed opacity-50", className)}
        onClick={() => {
          if (ref) {
