...
export default function Map({
  ...
  const [id] = useState(new Date().getMilliseconds() * Math.random());
  useEffect(() => {
    ...
    if (marker) {
      if (Array.isArray(marker)) {
        marker.map((markerItem) => {
          const MARKER = L.marker([markerItem.lat, markerItem.lng], {
            icon: getIcon(markerItem.iconUrl || "/svg/fallback_icon.svg"),
          }).addTo(map);
          MARKER.bindPopup(
            ReactDomServer.renderToString(<div className="">{markerItem.content}</div>),
          );
        ...
        MARKER.bindPopup(ReactDomServer.renderToString(<div className="">{marker.content}</div>));
      }
    }
  });
