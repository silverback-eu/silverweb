...
function CategoryButton({ category }: { category: string }): JSX.Element {
  ...
  return (
    <Button
      className="h-auto text-ellipsis p-0"
      onClick={() => {
...
export const columnsProject: ColumnDef<Project>[] = [
  {
    accessorKey: "cover",
    cell: ({ row }) => (
      <div className="relative h-[268px] w-[278px] overflow-hidden rounded-lg">
        <Image
          about="All Photos are from unsplash https://unsplash.com"
          alt={(row.getValue("cover") as Project["cover"]).alt}
          className="z-10 object-cover transition-all duration-500 hover:scale-105"
          fill
          loading="lazy"
          resource="unsplash"
          sizes="100%"
          src={(row.getValue("cover") as Project["cover"]).url}
        />
  ...
  },
  {
    accessorKey: "name",
    cell: ({ row }) => (
      <div className="mt-3 line-clamp-1 text-lg font-bold">
        {row.getValue("name")}
      </div>
    ),
  },
...

  {
    ...
    cell: ({ row }) => (
      <div className="line-clamp-1 text-sm font-bold opacity-70">
        {(row.getValue("location") as Project["location"]).city},{" "}
        {(row.getValue("location") as Project["location"]).country}
      </div>
    ),
    filterFn: (row, id, value) =>
      (value as string[]).includes(
        (row.getValue(id) as Project["location"]).country.toLowerCase(),
      ),
  },
  {
